 ...
 */
export const tenantRateLimitMiddleware = (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.tenant) {
      return next();
    }
    ...
    const _clientId = req.ip || req.connection.remoteAddress || 'unknown';
    const now = Date.now();
    const _windowStart = now - rateLimiting.windowMs;
