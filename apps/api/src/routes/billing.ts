...
function resolveUserTenantId(req: Request): string | undefined {
  const tenant = req.user?.tenant;
  if (!tenant) return undefined;
  if (typeof tenant === 'object' && tenant !== null && '_id' in tenant) {
    return (tenant as { _id: { toString(): string } })._id.toString();
  }
...
router.get('/plan', async (req: Request, res: Response) => {
  try {
    ...
    const userTenantId = resolveUserTenantId(req);
    const isSuperAdmin = req.user.role === UserRole.SUPER_ADMIN;

    if (!isSuperAdmin && userTenantId !== tenantId) {
      return res.status(403).json({ error: 'Access denied to requested tenant billing' });
    }
