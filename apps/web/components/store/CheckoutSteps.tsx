      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <li key={label} className="flex items-center gap-2 flex-1">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center ${active || done ? 'text-white' : 'text-secondary'}`}
              style={{ backgroundColor: active || done ? 'var(--color-blue)' : 'var(--color-fill-tertiary)' }}
            >
              {step}
            </span>
