...
| `SessionMonitor` | `apps/web/components/auth/SessionMonitor.tsx` | Polls JWT expiry; redirects with `reason=session_expired` |
...
`SessionMonitor` runs globally. `AuthGuard` wraps all pages and redirects unauthenticated users from protected prefixes.
...
- Reuses `.ios
ios-spinner`, `.ios-btn-plain` for dismiss
