...
## Addendum (2026-07-22, later same day): merged to main, then found one more build-blocker

The infra branch above merged into `main` as PR #423, on top of `main` having moved fast — a large batch of independent feature work had landed since this review started (a new `@luxgen/config` package, Redis-backed tenant caching, agent/billing/storefront/automation/mobile packages, and a separate PR that had already fixed several of the same bugs this review found, independently and in some cases more thoroughly - see the merge-conflict resolution notes; taking `main`'s side was the right call almost everywhere).

After merging, I re-ran the actual production build (`tsc`, not `--noEmit`) against the merged `main` and found a real, previously-uncounted problem: **43 pre-existing type errors**, all in files this work never touched — newer `packages/db` schemas (`automation.ts`, `automation-template.ts`, `business-listing.ts`, `connection.ts`, `custom-role.ts`, `enrollment.ts`, `learner-subscription.ts`, `mcp-api-key.ts`, `project-item.ts`, `storefront-bundle.ts`, `usage.ts`) plus a handful in `apps/api/src` (`config/tenants/index.ts`, `routes/notifications.ts`, `routes/tenantConfig.ts`, `schema/automation/resolvers.ts`, `services/activityEventService.ts`, `services/analyticsService.ts`, `services/groupService.ts`, `services/orderRowsService.ts`, `utils/jwt.ts`). Same category as `groupService.ts`'s already-flagged issue: mongoose@7.x type-definition gaps (`ObjectId.isValid`, `unique`/`sparse` missing from `IndexOptions`, schema `type:` array shape mismatches) plus a few unrelated implicit-`any`/argument-count errors.

**Why this mattered for deployment specifically:** `tsc` still *emits* working `.js` output despite these errors (`noEmitOnError` isn't set) — confirmed `dist/apps/api/src/index.js` exists and is correct even with all 43 errors present. But `tsc` still *exits non-zero* when it reports errors, and the Dockerfile's build step (`npx turbo run build`) treats any non-zero task exit as a failed pipeline and aborts the whole image build. So `docker build .` would have failed today, not because the app is broken, but purely because of this exit-code mismatch.

**Fix (with explicit sign-off to make this call):** `apps/api/scripts/build-tolerant.js` wraps `tsc` — runs it, and passes (exit 0) only if the compiled entrypoint actually exists *and* the error count is at or below a tracked baseline (43, matching exactly what's described above). If a *new* error shows up beyond that baseline, or the entrypoint fails to emit at all, it still fails hard. `apps/api/package.json`'s `"build"` script now points at this wrapper; the old direct `tsc` is preserved as `"build:strict"` for anyone who wants the unfiltered signal (e.g. a future cleanup pass). This is a deliberate, narrow, regression-guarded unblock — not a blanket "ignore type errors" switch — and it's scoped to retire itself: fix the 43 tracked errors (or downgrade a specific one to a documented `@ts-nocheck`/cast, same pattern as `groupService.ts`), lower `KNOWN_ERROR_BASELINE` as they're fixed, and once it hits 0, delete the wrapper and revert `"build"` to plain `tsc`.

This is now the single largest piece of tracked-but-deferred debt in the repo. It doesn't block deploying, but it should be the first thing tackled once the app is actually running on AWS — every day it sits at 43, it gets easier for a genuine new bug to hide in the noise of "expected" errors.
