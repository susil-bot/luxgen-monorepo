...
> **Pattern source:** `docs/SIDEBAR_REDESIGN.md` (sidebar), `CODEBASE.md` (full repo map)
...
interface EnrolledCourse {
  ...
  thumbnail: string; progress: number;   // 0–100
  duration: string; lastAccessedAt: string;
}
interface Certificate {
  id: string; courseTitle: string; issuedAt: string;
  pdfUrl: string;
}
```
...
| D2 | System prompt pre-filled from `CODEBASE.md` first-action instruction |
...
interface AutomationRun {
  ...
  triggeredAt: string; status: 'success' | 'error' | 'running';
