# 45-health.auto.test.ts

## File Path

`apps/agent-worker/src/vitest-auto/45-health.auto.test.ts`
...
- Walk me through 45-health.auto.test.ts line by line.
...
```bash
rg "45-health.auto.test" apps packages --glob '*.{ts,tsx}'
```
