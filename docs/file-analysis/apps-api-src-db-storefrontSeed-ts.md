...
`apps/api/src/db/storefrontSeed.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `ensureDemoStorefrontCourses`?"


## Interview Questions

- Walk me through ensureDemoStorefrontCourses line by line.
...
```bash
rg "storefrontSeed" apps packages --glob '*.{ts,tsx}'
```
