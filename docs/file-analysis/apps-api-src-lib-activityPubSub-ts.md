...
`apps/api/src/lib/activityPubSub.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `timelineTopic`?"

### `publishActivityEvent`

- **Approx. line:** 30
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `publishActivityEvent`?"


## Interview Questions

- Walk me through ACTIVITY_EVENT_ADDED line by line.
...
```bash
rg "activityPubSub" apps packages --glob '*.{ts,tsx}'
```
