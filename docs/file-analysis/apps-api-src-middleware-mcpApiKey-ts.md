...
`apps/api/src/middleware/mcpApiKey.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `mcpApiKeyMiddleware`?"


## Interview Questions

- Walk me through mcpApiKeyMiddleware line by line.
...
```bash
rg "mcpApiKey" apps packages --glob '*.{ts,tsx}'
```
