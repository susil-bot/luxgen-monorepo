# backfill-course-commerce.ts

## File Path

`apps/api/src/scripts/backfill-course-commerce.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `backfillCourseCommerce`?"


## Interview Questions

- Walk me through backfillCourseCommerce line by line.
...
```bash
rg "backfill-course-commerce" apps packages --glob '*.{ts,tsx}'
```
