# enrollmentStatusSummaryService.ts

## File Path

`apps/api/src/services/enrollmentStatusSummaryService.ts`
...
- `enrollmentStatusSummaryService`
...
- Walk me through enrollmentStatusSummaryService line by line.
...
```bash
rg "enrollmentStatusSummaryService" apps packages --glob '*.{ts,tsx}'
```
