# inAppNotificationsService.ts

## File Path

`apps/api/src/services/inAppNotificationsService.ts`
...
- Walk me through inAppNotificationsService line by line.
...
```bash
rg "inAppNotificationsService" apps packages --glob '*.{ts,tsx}'
```
