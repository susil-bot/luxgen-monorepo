...
`apps/api/src/services/listingSubscriptionService.ts`
...
- Walk me through ListingSubscriptionService line by line.
...
```bash
rg "listingSubscriptionService" apps packages --glob '*.{ts,tsx}'
```
