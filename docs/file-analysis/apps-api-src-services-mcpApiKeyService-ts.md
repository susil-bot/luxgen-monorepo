...
`apps/api/src/services/mcpApiKeyService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `hashMcpApiKey`?"


## Interview Questions

- Walk me through hashMcpApiKey line by line.
...
```bash
rg "mcpApiKeyService" apps packages --glob '*.{ts,tsx}'
```
