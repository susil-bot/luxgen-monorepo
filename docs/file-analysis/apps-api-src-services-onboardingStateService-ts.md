# onboardingStateService.ts

## File Path

`apps/api/src/services/onboardingStateService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `completed`?"


## Interview Questions

- Walk me through onboardingStateService line by line.
...
```bash
rg "onboardingStateService" apps packages --glob '*.{ts,tsx}'
```
