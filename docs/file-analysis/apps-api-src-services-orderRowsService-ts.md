...
`apps/api/src/services/orderRowsService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `listOrderRows`?"

### `students`

- **Approx. line:** 64
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `students`?"


## Interview Questions

- Walk me through ApiOrderRow line by line.
...
```bash
rg "orderRowsService" apps packages --glob '*.{ts,tsx}'
```
