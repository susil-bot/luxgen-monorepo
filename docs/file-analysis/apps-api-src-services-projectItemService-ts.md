# projectItemService.ts

## File Path

`apps/api/src/services/projectItemService.ts`
...
- Walk me through CreateProjectItemInput line by line.
...
```bash
rg "projectItemService" apps packages --glob '*.{ts,tsx}'
```
