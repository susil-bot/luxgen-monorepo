# storeCatalogFacetsService.ts

## File Path

`apps/api/src/services/storeCatalogFacetsService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `cat`?"


## Interview Questions

- Walk me through storeCatalogFacetsService line by line.
...
```bash
rg "storeCatalogFacetsService" apps packages --glob '*.{ts,tsx}'
```
