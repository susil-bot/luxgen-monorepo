...
`apps/api/src/services/storefrontService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `ensureDemoStorefrontBundles`?"


## Interview Questions

- Walk me through StorefrontService line by line.
...
```bash
rg "storefrontService" apps packages --glob '*.{ts,tsx}'
```
