...
`apps/api/src/services/usageService.ts`
...
- **Workspace packages:** @luxgen/db, @luxgen/billing
...
- **Interview:** "What are inputs, outputs, and side effects of `Stripe`?"


## Interview Questions

- Walk me through UsageService line by line.
...
```bash
rg "usageService" apps packages --glob '*.{ts,tsx}'
```
