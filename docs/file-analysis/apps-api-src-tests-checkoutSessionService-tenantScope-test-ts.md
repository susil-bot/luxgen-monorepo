# checkoutSessionService.tenantScope.test.ts

## File Path

`apps/api/src/tests/checkoutSessionService.tenantScope.test.ts`
...
- Walk me through checkoutSessionService.tenantScope.test.ts line by line.
...
```bash
rg "checkoutSessionService.tenantScope.test" apps packages --glob '*.{ts,tsx}'
```
