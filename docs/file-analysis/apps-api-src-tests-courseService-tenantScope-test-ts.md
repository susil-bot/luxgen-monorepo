# courseService.tenantScope.test.ts

## File Path

`apps/api/src/tests/courseService.tenantScope.test.ts`
...
- Walk me through courseService.tenantScope.test.ts line by line.
...
```bash
rg "courseService.tenantScope.test" apps packages --glob '*.{ts,tsx}'
```
