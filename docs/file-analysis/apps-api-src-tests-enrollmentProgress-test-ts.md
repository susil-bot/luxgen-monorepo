# enrollmentProgress.test.ts

## File Path

`apps/api/src/tests/enrollmentProgress.test.ts`
...
- Walk me through enrollmentProgress.test.ts line by line.
...
```bash
rg "enrollmentProgress.test" apps packages --glob '*.{ts,tsx}'
```
