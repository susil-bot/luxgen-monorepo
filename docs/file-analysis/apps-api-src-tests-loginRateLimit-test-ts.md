# loginRateLimit.test.ts — Brief + Junior Q&A

**Path:** `apps/api/src/tests/loginRateLimit.test.ts` (85 lines)  
...
**ANS.** Search imports: `rg "loginRateLimit.test" apps packages --glob '*.{ts,tsx}'`.
