# projectItem.moveStatus.test.ts

## File Path

`apps/api/src/tests/projectItem.moveStatus.test.ts`
...
- `@luxgen/db`
- `../../src/services/projectItemService`
...
- Walk me through projectItem.moveStatus.test.ts line by line.
...
```bash
rg "projectItem.moveStatus.test" apps packages --glob '*.{ts,tsx}'
```
