...
`apps/api/src/utils/accountStatus.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `isAccountActive`?"


## Interview Questions

- Walk me through isAccountActive line by line.
...
```bash
rg "accountStatus" apps packages --glob '*.{ts,tsx}'
```
