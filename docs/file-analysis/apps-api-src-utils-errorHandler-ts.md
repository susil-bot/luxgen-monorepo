...
- **Interview:** "What are inputs, outputs, and side effects of `errorHandler`?"

### `notFoundHandler`

- **Approx. line:** 53
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `notFoundHandler`?"


## Interview Questions

- Walk me through AppError line by line.
...
```bash
rg "errorHandler" apps packages --glob '*.{ts,tsx}'
```
