...
`apps/api/src/utils/tenantNotFound.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `tenantLinks`?"

### `renderTenantNotFound`

- **Approx. line:** 16
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `renderTenantNotFound`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through renderTenantNotFound line by line.
...
```bash
rg "tenantNotFound" apps packages --glob '*.{ts,tsx}'
```
