# 11-userResolver.auto.test.ts

## File Path

`apps/api/src/vitest-auto/11-userResolver.auto.test.ts`
...
- Walk me through 11-userResolver.auto.test.ts line by line.
...
```bash
rg "11-userResolver.auto.test" apps packages --glob '*.{ts,tsx}'
```
