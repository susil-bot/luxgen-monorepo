# 12-courseResolver.auto.test.ts

## File Path

`apps/api/src/vitest-auto/12-courseResolver.auto.test.ts`
...
- Walk me through 12-courseResolver.auto.test.ts line by line.
...
```bash
rg "12-courseResolver.auto.test" apps packages --glob '*.{ts,tsx}'
```
