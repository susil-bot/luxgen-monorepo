# 13-enrollmentResolver.auto.test.ts

## File Path

`apps/api/src/vitest-auto/13-enrollmentResolver.auto.test.ts`
...
- Walk me through 13-enrollmentResolver.auto.test.ts line by line.
...
```bash
rg "13-enrollmentResolver.auto.test" apps packages --glob '*.{ts,tsx}'
```
