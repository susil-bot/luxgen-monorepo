# 14-automationResolverExport.auto.test.ts

## File Path

`apps/api/src/vitest-auto/14-automationResolverExport.auto.test.ts`
...
- Walk me through 14-automationResolverExport.auto.test.ts line by line.
...
```bash
rg "14-automationResolverExport.auto.test" apps packages --glob '*.{ts,tsx}'
```
