# 16-tenantResolver.auto.test.ts

## File Path

`apps/api/src/vitest-auto/16-tenantResolver.auto.test.ts`
...
- Walk me through 16-tenantResolver.auto.test.ts line by line.
...
```bash
rg "16-tenantResolver.auto.test" apps packages --glob '*.{ts,tsx}'
```
