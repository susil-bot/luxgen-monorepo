# 17-learnerResolver.auto.test.ts

## File Path

`apps/api/src/vitest-auto/17-learnerResolver.auto.test.ts`
...
- Walk me through 17-learnerResolver.auto.test.ts line by line.
...
```bash
rg "17-learnerResolver.auto.test" apps packages --glob '*.{ts,tsx}'
```
