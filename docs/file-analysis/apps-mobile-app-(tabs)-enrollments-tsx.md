...
`apps/mobile/app/(tabs)/enrollments.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `myEnrollments`?"


## Interview Questions

- Walk me through EnrollmentsScreen line by line.
...
```bash
rg "enrollments" apps packages --glob '*.{ts,tsx}'
```
