...
- Walk me through babel.config.js line by line.
...
- **Why this way?** Colocated with feature domain (`apps/mobile/babel.config.js`).
...
```bash
rg "babel.config" apps packages --glob '*.{ts,tsx}'
```
