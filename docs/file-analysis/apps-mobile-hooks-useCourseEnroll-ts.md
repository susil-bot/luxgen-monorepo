...
`apps/mobile/hooks/useCourseEnroll.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useCourseEnroll`?"
...
- Walk me through useCourseEnroll line by line.
...
```bash
rg "useCourseEnroll" apps packages --glob '*.{ts,tsx}'
```
