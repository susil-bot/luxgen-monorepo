...
`apps/mobile/hooks/useOtaUpdates.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useOtaUpdates`?"
...
- Walk me through useOtaUpdates line by line.
...
```bash
rg "useOtaUpdates" apps packages --glob '*.{ts,tsx}'
```
