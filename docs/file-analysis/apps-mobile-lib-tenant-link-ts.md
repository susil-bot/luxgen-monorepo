...
`apps/mobile/lib/tenant-link.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `parseTenantFromUrl`?"

### `buildTenantLoginLink`

- **Approx. line:** 27
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `buildTenantLoginLink`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through parseTenantFromUrl line by line.
...
```bash
rg "tenant-link" apps packages --glob '*.{ts,tsx}'
```
