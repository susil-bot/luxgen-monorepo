...
- Walk me through metro.config.js line by line.
...
- **Why this way?** Colocated with feature domain (`apps/mobile/metro.config.js`).
...
```bash
rg "metro.config" apps packages --glob '*.{ts,tsx}'
```
