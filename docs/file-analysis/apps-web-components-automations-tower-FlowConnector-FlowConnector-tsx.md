...
`apps/web/components/automations/tower/FlowConnector/FlowConnector.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `FlowConnector`?"
...
```bash
rg "FlowConnector" apps packages --glob '*.{ts,tsx}'
```
