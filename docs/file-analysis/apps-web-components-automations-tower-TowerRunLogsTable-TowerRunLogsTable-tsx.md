# TowerRunLogsTable.tsx

## File Path

`apps/web/components/automations/tower/TowerRunLogsTable/TowerRunLogsTable.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `TowerRunLogsTable`?"
...
```bash
rg "TowerRunLogsTable" apps packages --glob '*.{ts,tsx}'
```
