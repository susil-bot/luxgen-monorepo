...
`apps/web/components/common/PageStates.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `PageLoadingState`?"

### `PageEmptyState`

- **Approx. line:** 27
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `PageEmptyState`?"


## Interview Questions

- Walk me through PageLoadingState line by line.
...
```bash
rg "PageStates" apps packages --glob '*.{ts,tsx}'
```
