...
`apps/web/components/dashboard/DashboardBanner.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `DashboardBanner`?"


## Interview Questions

- Walk me through DashboardBannerItem line by line.
...
```bash
rg "DashboardBanner" apps packages --glob '*.{ts,tsx}'
```
