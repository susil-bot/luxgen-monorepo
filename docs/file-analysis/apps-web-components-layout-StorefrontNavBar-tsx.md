...
`apps/web/components/layout/StorefrontNavBar.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `StorefrontNavBar`?"


## Interview Questions

- How do you avoid duplicate navigation callbacks (onNavigate vs onItemClick)?
- Walk me through StorefrontNavBar line by line.
...
```bash
rg "StorefrontNavBar" apps packages --glob '*.{ts,tsx}'
```
