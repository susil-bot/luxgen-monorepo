...
`apps/web/components/media/OptimizedImage.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OptimizedImage`?"


## Interview Questions

- Walk me through OptimizedImage line by line.
...
```bash
rg "OptimizedImage" apps packages --glob '*.{ts,tsx}'
```
