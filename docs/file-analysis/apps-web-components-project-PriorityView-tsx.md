...
`apps/web/components/project/PriorityView.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `PriorityView`?"


## Interview Questions

- Walk me through PriorityView line by line.
...
```bash
rg "PriorityView" apps packages --glob '*.{ts,tsx}'
```
