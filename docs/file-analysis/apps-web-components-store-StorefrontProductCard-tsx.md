...
`apps/web/components/store/StorefrontProductCard.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `StorefrontProductCard`?"


## Interview Questions

- Walk me through StorefrontProductCard line by line.
...
```bash
rg "StorefrontProductCard" apps packages --glob '*.{ts,tsx}'
```
