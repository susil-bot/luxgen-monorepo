...
- **Interview:** "What are inputs, outputs, and side effects of `TenantSwitcher`?"

### `handleTenantChange`

- **Approx. line:** 18
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `handleTenantChange`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through TenantSwitcher line by line.
...
```bash
rg "TenantSwitcher" apps packages --glob '*.{ts,tsx}'
```
