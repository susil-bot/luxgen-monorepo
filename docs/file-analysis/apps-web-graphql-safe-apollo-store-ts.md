# safe-apollo-store.ts

## File Path

`apps/web/graphql/safe-apollo-store.ts`
...
- `safeClearApolloStore`
- `safeResetApolloStore`
...
- **Interview:** "What are inputs, outputs, and side effects of `safeClearApolloStore`?"

### `safeResetApolloStore`

- **Approx. line:** 13
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `safeResetApolloStore`?"


## Interview Questions

- Walk me through safeClearApolloStore line by line.
...
```bash
rg "safe-apollo-store" apps packages --glob '*.{ts,tsx}'
```
