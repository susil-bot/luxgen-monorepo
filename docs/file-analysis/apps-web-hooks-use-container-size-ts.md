...
`apps/web/hooks/use-container-size.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useContainerSize`?"
...
- Walk me through useContainerSize line by line.
...
```bash
rg "use-container-size" apps packages --glob '*.{ts,tsx}'
```
