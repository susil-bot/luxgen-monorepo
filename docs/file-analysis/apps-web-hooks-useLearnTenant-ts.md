...
`apps/web/hooks/useLearnTenant.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useLearnTenant`?"
...
- Walk me through useLearnTenant line by line.
...
```bash
rg "useLearnTenant" apps packages --glob '*.{ts,tsx}'
```
