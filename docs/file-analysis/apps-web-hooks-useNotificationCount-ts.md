...
`apps/web/hooks/useNotificationCount.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useNotificationCount`?"
...
- Walk me through useNotificationCount line by line.
...
```bash
rg "useNotificationCount" apps packages --glob '*.{ts,tsx}'
```
