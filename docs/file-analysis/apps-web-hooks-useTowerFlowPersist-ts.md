# useTowerFlowPersist.ts

## File Path

`apps/web/hooks/useTowerFlowPersist.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useTowerFlowPersist`?"
...
- Walk me through TowerSaveState line by line.
...
```bash
rg "useTowerFlowPersist" apps packages --glob '*.{ts,tsx}'
```
