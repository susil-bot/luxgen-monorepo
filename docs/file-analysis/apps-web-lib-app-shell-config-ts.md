...
`apps/web/lib/app-shell-config.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useAppShellConfig`?"
...
- Walk me through useAppShellConfig line by line.
...
```bash
rg "app-shell-config" apps packages --glob '*.{ts,tsx}'
```
