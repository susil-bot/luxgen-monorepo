...
- **Interview:** "What are inputs, outputs, and side effects of `downloadCsv`?"

### `exportUsersCsv`

- **Approx. line:** 13
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `exportUsersCsv`?"


## Interview Questions

- Walk me through downloadCsv line by line.
...
```bash
rg "export-csv" apps packages --glob '*.{ts,tsx}'
```
