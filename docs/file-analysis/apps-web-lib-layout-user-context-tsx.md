# layout-user-context.tsx

## File Path

`apps/web/lib/layout-user-context.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `LayoutUserProvider`?"

### `useLayoutUserFromContext`

- **Approx. line:** 44
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `useLayoutUserFromContext`?"
...
- Walk me through LayoutUserProvider line by line.
...
```bash
rg "layout-user-context" apps packages --glob '*.{ts,tsx}'
```
