...
- **Interview:** "What are inputs, outputs, and side effects of `fetchTenantBilling`?"

### `requirePlanFeature`

- **Approx. line:** 20
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `requirePlanFeature`?"


## Interview Questions

- Walk me through TenantBillingSnapshot line by line.
...
```bash
rg "plan-gate" apps packages --glob '*.{ts,tsx}'
```
