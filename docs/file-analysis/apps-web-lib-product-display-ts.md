...
`apps/web/lib/product-display.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `courseToProductRow`?"

### `formatProductDate`

- **Approx. line:** 44
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `formatProductDate`?"
...
```bash
rg "product-display" apps packages --glob '*.{ts,tsx}'
```
