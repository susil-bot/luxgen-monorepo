...
`apps/web/lib/project-workflows.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `getWorkflowTemplate`?"

### `workflowItemToCreateInput`

- **Approx. line:** 174
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `workflowItemToCreateInput`?"


## Interview Questions

- Walk me through ProjectWorkflowSeedItem line by line.
...
```bash
rg "project-workflows" apps packages --glob '*.{ts,tsx}'
```
