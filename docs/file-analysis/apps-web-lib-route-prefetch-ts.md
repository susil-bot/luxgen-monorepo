...
`apps/web/lib/route-prefetch.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `prefetchCommonRoutes`?"


## Interview Questions

- Walk me through prefetchCommonRoutes line by line.
...
```bash
rg "route-prefetch" apps packages --glob '*.{ts,tsx}'
```
