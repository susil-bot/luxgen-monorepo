...
`apps/web/lib/store-categories.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `categoryLabel`?"


## Interview Questions

- Walk me through STORE_CATEGORIES line by line.
...
```bash
rg "store-categories" apps packages --glob '*.{ts,tsx}'
```
