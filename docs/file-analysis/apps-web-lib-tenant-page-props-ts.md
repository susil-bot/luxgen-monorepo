# tenant-page-props.ts

## File Path

`apps/web/lib/tenant-page-props.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `resolvePageTenant`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through TenantPageProps line by line.
...
```bash
rg "tenant-page-props" apps packages --glob '*.{ts,tsx}'
```
