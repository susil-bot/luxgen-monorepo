...
`apps/web/lib/use-client-mounted.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useClientMounted`?"
...
- Walk me through useClientMounted line by line.
...
```bash
rg "use-client-mounted" apps packages --glob '*.{ts,tsx}'
```
