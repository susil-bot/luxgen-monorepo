# use-customer-notes.ts

## File Path

`apps/web/lib/use-customer-notes.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useCustomerNotes`?"
...
- Walk me through useCustomerNotes line by line.
...
```bash
rg "use-customer-notes" apps packages --glob '*.{ts,tsx}'
```
