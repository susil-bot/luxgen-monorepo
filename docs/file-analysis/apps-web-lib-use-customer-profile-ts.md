...
`apps/web/lib/use-customer-profile.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useCustomerProfile`?"
...
- Walk me through useCustomerProfile line by line.
...
```bash
rg "use-customer-profile" apps packages --glob '*.{ts,tsx}'
```
