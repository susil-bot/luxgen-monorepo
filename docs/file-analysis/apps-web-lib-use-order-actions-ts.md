...
`apps/web/lib/use-order-actions.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useOrderActions`?"
...
- Walk me through useOrderActions line by line.
...
```bash
rg "use-order-actions" apps packages --glob '*.{ts,tsx}'
```
