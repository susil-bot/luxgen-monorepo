# 35-auth-routes.auto.test.ts

## File Path

`apps/web/lib/vitest-auto/35-auth-routes.auto.test.ts`
...
- Walk me through 35-auth-routes.auto.test.ts line by line.
...
```bash
rg "35-auth-routes.auto.test" apps packages --glob '*.{ts,tsx}'
```
