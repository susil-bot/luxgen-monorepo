...
- **Interview:** "What are inputs, outputs, and side effects of `middleware`?"
...
- Walk me through middleware line by line.
...
- **Why this way?** Colocated with feature domain (`apps/web/middleware.ts`).
...
```bash
rg "middleware" apps packages --glob '*.{ts,tsx}'
```
