...
- **Interview:** "What are inputs, outputs, and side effects of `endStream`?"

### `poll`

- **Approx. line:** 54
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `poll`?"
...
```bash
rg "stream" apps packages --glob '*.{ts,tsx}'
```
