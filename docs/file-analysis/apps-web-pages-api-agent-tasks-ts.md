...
`apps/web/pages/api/agent/tasks.ts`
...
- **Workspace packages:** @luxgen/agent, @luxgen/config
...
- **Interview:** "What are inputs, outputs, and side effects of `sessionId`?"
...
```bash
rg "tasks" apps packages --glob '*.{ts,tsx}'
```
