...
- Walk me through postcss.config.js line by line.
...
- **Why this way?** Colocated with feature domain (`apps/web/postcss.config.js`).
...
```bash
rg "postcss.config" apps packages --glob '*.{ts,tsx}'
```
