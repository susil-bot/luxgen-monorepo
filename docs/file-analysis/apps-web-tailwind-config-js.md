...
- Walk me through tailwind.config.js line by line.
...
- **Why this way?** Colocated with feature domain (`apps/web/tailwind.config.js`).
...
```bash
rg "tailwind.config" apps packages --glob '*.{ts,tsx}'
```
