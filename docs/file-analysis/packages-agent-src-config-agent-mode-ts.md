...
`packages/agent/src/config/agent-mode.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `getAgentConfig`?"

### `shouldUseGitWorktree`

- **Approx. line:** 20
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `shouldUseGitWorktree`?"

### `mode`

- **Approx. line:** 11
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `mode`?"


## Interview Questions

- Walk me through AgentDeploymentMode line by line.
...
```bash
rg "agent-mode" apps packages --glob '*.{ts,tsx}'
```
