...
`packages/agent/src/validation/policy.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `getValidationPolicy`?"

### `validationBlocksCommit`

- **Approx. line:** 12
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `validationBlocksCommit`?"


## Interview Questions

- Walk me through getValidationPolicy line by line.
...
```bash
rg "policy" apps packages --glob '*.{ts,tsx}'
```
