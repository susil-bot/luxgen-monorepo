# attendanceTracker.ts

## File Path

`packages/core/src/scheduler/attendanceTracker.ts`
...
- Walk me through AttendanceRecord line by line.
...
```bash
rg "attendanceTracker" apps packages --glob '*.{ts,tsx}'
```
