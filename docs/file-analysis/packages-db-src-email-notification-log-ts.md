# email-notification-log.ts

## File Path

`packages/db/src/email-notification-log.ts`
...
- Walk me through ListingEmailTemplate line by line.
...
```bash
rg "email-notification-log" apps packages --glob '*.{ts,tsx}'
```
