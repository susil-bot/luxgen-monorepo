...
`packages/db/src/tenant-config/default.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `mergeTenantConfig`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through TenantTheme line by line.
...
```bash
rg "default" apps packages --glob '*.{ts,tsx}'
```
