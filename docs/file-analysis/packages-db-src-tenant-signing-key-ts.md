# tenant-signing-key.ts

## File Path

`packages/db/src/tenant-signing-key.ts`
...
- Walk me through TenantSigningKeyStatus line by line.
...
```bash
rg "tenant-signing-key" apps packages --glob '*.{ts,tsx}'
```
