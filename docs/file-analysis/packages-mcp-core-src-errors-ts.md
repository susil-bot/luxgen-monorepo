...
- **Interview:** "What are inputs, outputs, and side effects of `formatToolSuccess`?"

### `formatToolError`

- **Approx. line:** 32
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `formatToolError`?"


## Interview Questions

- Walk me through McpTextContent line by line.
...
```bash
rg "errors" apps packages --glob '*.{ts,tsx}'
```
