...
`packages/mcp-core/src/rate-limit.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `rateLimitKey`?"


## Interview Questions

- Walk me through RateLimitResult line by line.
...
```bash
rg "rate-limit" apps packages --glob '*.{ts,tsx}'
```
