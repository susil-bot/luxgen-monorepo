# commerce-handlers.ts

## File Path

`packages/mcp-core/src/tools/commerce-handlers.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `handleCommerceTool`?"


## Interview Questions

- Walk me through handleCommerceTool line by line.
...
```bash
rg "commerce-handlers" apps packages --glob '*.{ts,tsx}'
```
