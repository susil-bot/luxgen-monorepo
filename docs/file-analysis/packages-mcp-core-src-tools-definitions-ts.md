...
`packages/mcp-core/src/tools/definitions.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `allToolDefinitions`?"


## Interview Questions

- Walk me through allToolDefinitions line by line.
...
```bash
rg "definitions" apps packages --glob '*.{ts,tsx}'
```
