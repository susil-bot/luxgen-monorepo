...
`packages/mcp-core/src/tools/scopes.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `toolRequiresWrite`?"

### `isToolAllowed`

- **Approx. line:** 20
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `isToolAllowed`?"

### `filterToolsByScope`

- **Approx. line:** 26
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `filterToolsByScope`?"
...
```bash
rg "scopes" apps packages --glob '*.{ts,tsx}'
```
