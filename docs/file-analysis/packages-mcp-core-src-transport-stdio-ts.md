...
`packages/mcp-core/src/transport/stdio.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `runStdioServer`?"


## Interview Questions

- Walk me through runStdioServer line by line.
...
```bash
rg "stdio" apps packages --glob '*.{ts,tsx}'
```
