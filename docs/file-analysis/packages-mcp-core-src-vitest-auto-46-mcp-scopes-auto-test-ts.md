# 46-mcp-scopes.auto.test.ts

## File Path

`packages/mcp-core/src/vitest-auto/46-mcp-scopes.auto.test.ts`
...
- Walk me through 46-mcp-scopes.auto.test.ts line by line.
...
```bash
rg "46-mcp-scopes.auto.test" apps packages --glob '*.{ts,tsx}'
```
