...
`packages/shared/src/tenant/TenantConfigService.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `cacheTime`?"


## Interview Questions

- How is tenant resolved: subdomain, header, or JWT claim?
- Walk me through TenantConfigServiceOptions line by line.
...
- **Why this way?** Colocated with feature domain (`packages/shared/src`).
...
```bash
rg "TenantConfigService" apps packages --glob '*.{ts,tsx}'
```
