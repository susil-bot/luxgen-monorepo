...
`packages/storefront/src/graphql/adapters.ts`
...
- **Approx. line:** 43
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `buildLearnifyCatalog`?"


## Interview Questions

- Walk me through filterPublishedCourses line by line.
...
```bash
rg "adapters" apps packages --glob '*.{ts,tsx}'
```
