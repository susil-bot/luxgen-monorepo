...
`packages/test-harness/src/fixtures/mongoose.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `mockObjectId`?"


## Interview Questions

- Walk me through mockObjectId line by line.
...
```bash
rg "mongoose" apps packages --glob '*.{ts,tsx}'
```
