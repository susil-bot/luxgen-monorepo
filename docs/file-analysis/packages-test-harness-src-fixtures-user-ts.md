...
`packages/test-harness/src/fixtures/user.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `mockIUser`?"


## Interview Questions

- Walk me through mockIUser line by line.
...
```bash
rg "user" apps packages --glob '*.{ts,tsx}'
```
