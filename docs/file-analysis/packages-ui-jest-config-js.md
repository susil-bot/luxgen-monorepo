...
- Walk me through jest.config.js line by line.
...
- **Why this way?** Colocated with feature domain (`packages/ui/jest.config.js`).
...
```bash
rg "jest.config" apps packages --glob '*.{ts,tsx}'
```
