# AIStudioSidekick.tsx

## File Path

`packages/ui/src/AIStudio/AIStudioSidekick.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `AIStudioSidekick`?"

### `AIStudioSidekickFooter`

- **Approx. line:** 86
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `AIStudioSidekickFooter`?"
...
- Walk me through AIStudioSidekickProps line by line.
...
```bash
rg "AIStudioSidekick" apps packages --glob '*.{ts,tsx}'
```
