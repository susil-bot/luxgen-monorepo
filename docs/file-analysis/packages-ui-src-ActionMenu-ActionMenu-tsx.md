...
`packages/ui/src/ActionMenu/ActionMenu.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `ActionMenu`?"

### `onPointerDown`

- **Approx. line:** 58
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `onPointerDown`?"
...
- Walk me through ActionMenuItem line by line.
...
```bash
rg "ActionMenu" apps packages --glob '*.{ts,tsx}'
```
