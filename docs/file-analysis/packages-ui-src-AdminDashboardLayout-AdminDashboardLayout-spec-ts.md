# AdminDashboardLayout.spec.ts

## File Path

`packages/ui/src/AdminDashboardLayout/AdminDashboardLayout.spec.ts`
...
- Walk me through AdminDashboardLayout.spec.ts line by line.
...
```bash
rg "AdminDashboardLayout.spec" apps packages --glob '*.{ts,tsx}'
```
