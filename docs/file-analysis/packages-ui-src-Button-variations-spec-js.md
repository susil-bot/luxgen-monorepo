...
`packages/ui/src/Button/variations.spec.js`
...
- **Interview:** "What are inputs, outputs, and side effects of `renderWithTheme`?"


## Interview Questions

- Why is this file side-effect only?
- Walk me through variations.spec.js line by line.
...
```bash
rg "variations.spec" apps packages --glob '*.{ts,tsx}'
```
