...
`packages/ui/src/Customer/CustomerCreateForm.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerCreateForm`?"


## Interview Questions

- Walk me through CustomerCreateFormProps line by line.
...
```bash
rg "CustomerCreateForm" apps packages --glob '*.{ts,tsx}'
```
