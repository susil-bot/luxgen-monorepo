...
`packages/ui/src/Customer/CustomerDetailHeader.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerDetailHeader`?"


## Interview Questions

- Walk me through CustomerDetailHeaderProps line by line.
...
```bash
rg "CustomerDetailHeader" apps packages --glob '*.{ts,tsx}'
```
