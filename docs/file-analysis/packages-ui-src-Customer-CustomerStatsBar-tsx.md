...
`packages/ui/src/Customer/CustomerStatsBar.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerStatsBar`?"


## Interview Questions

- Walk me through CustomerStatsBarProps line by line.
...
```bash
rg "CustomerStatsBar" apps packages --glob '*.{ts,tsx}'
```
