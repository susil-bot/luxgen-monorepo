...
`packages/ui/src/Customer/detail/BlocksSection/BlocksSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerBlocksSection`?"


## Interview Questions

- Walk me through CustomerBlocksSectionProps line by line.
...
```bash
rg "BlocksSection" apps packages --glob '*.{ts,tsx}'
```
