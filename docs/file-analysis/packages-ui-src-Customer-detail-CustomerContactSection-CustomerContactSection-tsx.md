...
`packages/ui/src/Customer/detail/CustomerContactSection/CustomerContactSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerContactSection`?"


## Interview Questions

- Walk me through CustomerContactSection line by line.
...
```bash
rg "CustomerContactSection" apps packages --glob '*.{ts,tsx}'
```
