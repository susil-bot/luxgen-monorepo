...
`packages/ui/src/Customer/detail/CustomerNotesSection/CustomerNotesSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerNotesSection`?"


## Interview Questions

- Walk me through CustomerNotesSection line by line.
...
```bash
rg "CustomerNotesSection" apps packages --glob '*.{ts,tsx}'
```
