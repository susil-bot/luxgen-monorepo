...
`packages/ui/src/Customer/detail/MarketingSection/MarketingSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `MarketingSection`?"
...
```bash
rg "MarketingSection" apps packages --glob '*.{ts,tsx}'
```
