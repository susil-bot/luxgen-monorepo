...
`packages/ui/src/Customer/list/CustomerListTable/CustomerListTable.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerListTable`?"


## Interview Questions

- Walk me through CustomerListTableProps line by line.
...
```bash
rg "CustomerListTable" apps packages --glob '*.{ts,tsx}'
```
