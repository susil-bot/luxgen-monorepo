...
`packages/ui/src/DataList/DataListPage.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `handleSearchCancel`?"


## Interview Questions

- Walk me through DataListPageProps line by line.
...
```bash
rg "DataListPage" apps packages --glob '*.{ts,tsx}'
```
