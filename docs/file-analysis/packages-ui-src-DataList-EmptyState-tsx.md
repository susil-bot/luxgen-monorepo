...
`packages/ui/src/DataList/EmptyState.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `DefaultSearchIcon`?"


## Interview Questions

- Walk me through EmptyState line by line.
...
```bash
rg "EmptyState" apps packages --glob '*.{ts,tsx}'
```
