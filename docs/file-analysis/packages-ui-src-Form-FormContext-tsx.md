...
`packages/ui/src/Form/FormContext.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `useFormContext`?"
...
- Walk me through FormProvider line by line.
...
```bash
rg "FormContext" apps packages --glob '*.{ts,tsx}'
```
