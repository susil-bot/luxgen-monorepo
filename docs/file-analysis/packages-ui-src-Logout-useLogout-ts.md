...
`packages/ui/src/Logout/useLogout.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useLogout`?"
...
- Walk me through UseLogoutOptions line by line.
...
```bash
rg "useLogout" apps packages --glob '*.{ts,tsx}'
```
