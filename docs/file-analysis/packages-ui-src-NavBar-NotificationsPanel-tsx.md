...
`packages/ui/src/NavBar/NotificationsPanel.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `NotificationsPanel`?"


## Interview Questions

- How do you avoid duplicate navigation callbacks (onNavigate vs onItemClick)?
- Walk me through NotificationsPanel line by line.
...
```bash
rg "NotificationsPanel" apps packages --glob '*.{ts,tsx}'
```
