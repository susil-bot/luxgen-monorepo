...
`packages/ui/src/NotFound/NotFound.spec.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `customActions`?"


## Interview Questions

- Why is this file side-effect only?
- Walk me through NotFound.spec.tsx line by line.
...
```bash
rg "NotFound.spec" apps packages --glob '*.{ts,tsx}'
```
