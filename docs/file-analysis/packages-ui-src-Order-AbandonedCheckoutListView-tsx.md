# AbandonedCheckoutListView.tsx

## File Path

`packages/ui/src/Order/AbandonedCheckoutListView.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `AbandonedCheckoutListView`?"

### `AbandonedIcon`

- **Approx. line:** 15
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `AbandonedIcon`?"


## Interview Questions

- Walk me through AbandonedCheckoutListViewProps line by line.
...
```bash
rg "AbandonedCheckoutListView" apps packages --glob '*.{ts,tsx}'
```
