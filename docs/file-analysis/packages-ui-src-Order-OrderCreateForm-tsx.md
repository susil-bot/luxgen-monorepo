...
`packages/ui/src/Order/OrderCreateForm.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OrderCreateForm`?"


## Interview Questions

- Walk me through OrderCreateStudentOption line by line.
...
```bash
rg "OrderCreateForm" apps packages --glob '*.{ts,tsx}'
```
