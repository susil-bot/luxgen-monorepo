...
`packages/ui/src/Order/OrderEditForm.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OrderEditForm`?"


## Interview Questions

- Walk me through OrderEditFormProps line by line.
...
```bash
rg "OrderEditForm" apps packages --glob '*.{ts,tsx}'
```
