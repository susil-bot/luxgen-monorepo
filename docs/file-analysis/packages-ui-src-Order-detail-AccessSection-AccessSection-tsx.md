...
`packages/ui/src/Order/detail/AccessSection/AccessSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `AccessSection`?"


## Interview Questions

- Walk me through AccessSectionProps line by line.
...
```bash
rg "AccessSection" apps packages --glob '*.{ts,tsx}'
```
