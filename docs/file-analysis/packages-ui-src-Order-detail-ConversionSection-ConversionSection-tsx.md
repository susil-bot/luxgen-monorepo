...
`packages/ui/src/Order/detail/ConversionSection/ConversionSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `ConversionSection`?"


## Interview Questions

- Walk me through ConversionSectionProps line by line.
...
```bash
rg "ConversionSection" apps packages --glob '*.{ts,tsx}'
```
