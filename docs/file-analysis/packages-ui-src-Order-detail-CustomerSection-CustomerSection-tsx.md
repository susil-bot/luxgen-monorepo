...
`packages/ui/src/Order/detail/CustomerSection/CustomerSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CustomerSection`?"


## Interview Questions

- Walk me through CustomerSectionProps line by line.
...
```bash
rg "CustomerSection" apps packages --glob '*.{ts,tsx}'
```
