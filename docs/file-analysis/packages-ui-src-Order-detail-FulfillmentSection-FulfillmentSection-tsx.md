...
`packages/ui/src/Order/detail/FulfillmentSection/FulfillmentSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `FulfillmentSection`?"


## Interview Questions

- Walk me through FulfillmentSectionProps line by line.
...
```bash
rg "FulfillmentSection" apps packages --glob '*.{ts,tsx}'
```
