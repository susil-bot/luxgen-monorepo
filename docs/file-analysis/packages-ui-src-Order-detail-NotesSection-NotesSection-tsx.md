...
`packages/ui/src/Order/detail/NotesSection/NotesSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `NotesSection`?"


## Interview Questions

- Walk me through NotesSectionProps line by line.
...
```bash
rg "NotesSection" apps packages --glob '*.{ts,tsx}'
```
