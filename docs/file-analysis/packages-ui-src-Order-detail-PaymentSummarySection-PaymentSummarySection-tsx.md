...
`packages/ui/src/Order/detail/PaymentSummarySection/PaymentSummarySection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `PaymentSummarySection`?"


## Interview Questions

- Walk me through PaymentSummarySectionProps line by line.
...
```bash
rg "PaymentSummarySection" apps packages --glob '*.{ts,tsx}'
```
