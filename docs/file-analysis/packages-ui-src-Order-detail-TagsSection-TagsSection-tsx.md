...
`packages/ui/src/Order/detail/TagsSection/TagsSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `TagsSection`?"


## Interview Questions

- Walk me through TagsSectionProps line by line.
...
```bash
rg "TagsSection" apps packages --glob '*.{ts,tsx}'
```
