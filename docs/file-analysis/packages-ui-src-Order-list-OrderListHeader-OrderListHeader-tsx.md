...
`packages/ui/src/Order/list/OrderListHeader/OrderListHeader.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OrderListHeader`?"


## Interview Questions

- Walk me through OrderListHeaderProps line by line.
...
```bash
rg "OrderListHeader" apps packages --glob '*.{ts,tsx}'
```
