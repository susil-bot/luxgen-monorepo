...
`packages/ui/src/Order/list/OrderListTable/OrderListTable.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OrderListTable`?"


## Interview Questions

- Walk me through OrderListTableProps line by line.
...
```bash
rg "OrderListTable" apps packages --glob '*.{ts,tsx}'
```
