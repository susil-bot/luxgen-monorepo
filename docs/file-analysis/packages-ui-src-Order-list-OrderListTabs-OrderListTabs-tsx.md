...
`packages/ui/src/Order/list/OrderListTabs/OrderListTabs.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `OrderListTabs`?"


## Interview Questions

- Walk me through OrderListTabsProps line by line.
...
```bash
rg "OrderListTabs" apps packages --glob '*.{ts,tsx}'
```
