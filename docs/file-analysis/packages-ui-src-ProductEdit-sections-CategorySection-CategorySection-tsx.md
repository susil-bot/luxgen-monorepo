...
`packages/ui/src/ProductEdit/sections/CategorySection/CategorySection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `CategorySection`?"


## Interview Questions

- Walk me through CategorySectionProps line by line.
...
```bash
rg "CategorySection" apps packages --glob '*.{ts,tsx}'
```
