...
`packages/ui/src/ProductEdit/sections/DeliverySection/DeliverySection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `DeliverySection`?"
...
```bash
rg "DeliverySection" apps packages --glob '*.{ts,tsx}'
```
