...
`packages/ui/src/ProductEdit/sections/InventorySection/InventorySection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `InventorySection`?"


## Interview Questions

- Walk me through InventorySectionProps line by line.
...
```bash
rg "InventorySection" apps packages --glob '*.{ts,tsx}'
```
