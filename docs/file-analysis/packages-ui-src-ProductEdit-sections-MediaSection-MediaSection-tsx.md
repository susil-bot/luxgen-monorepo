...
`packages/ui/src/ProductEdit/sections/MediaSection/MediaSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `MediaSection`?"


## Interview Questions

- Walk me through MediaSectionProps line by line.
...
```bash
rg "MediaSection" apps packages --glob '*.{ts,tsx}'
```
