...
`packages/ui/src/ProductEdit/sections/TitleDescriptionSection/SimpleRichTextEditor.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `SimpleRichTextEditor`?"

### `handleInput`

- **Approx. line:** 34
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `handleInput`?"


## Interview Questions

- Walk me through SimpleRichTextEditorProps line by line.
...
```bash
rg "SimpleRichTextEditor" apps packages --glob '*.{ts,tsx}'
```
