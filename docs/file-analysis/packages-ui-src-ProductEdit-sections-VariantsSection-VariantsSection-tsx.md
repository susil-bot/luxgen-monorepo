...
`packages/ui/src/ProductEdit/sections/VariantsSection/VariantsSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `VariantsSection`?"
...
```bash
rg "VariantsSection" apps packages --glob '*.{ts,tsx}'
```
