...
`packages/ui/src/ProductEdit/sidebar/SalesSection/SalesSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `SalesSection`?"


## Interview Questions

- Walk me through SalesSectionProps line by line.
...
```bash
rg "SalesSection" apps packages --glob '*.{ts,tsx}'
```
