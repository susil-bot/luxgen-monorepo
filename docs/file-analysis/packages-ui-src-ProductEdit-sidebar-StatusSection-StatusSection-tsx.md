...
`packages/ui/src/ProductEdit/sidebar/StatusSection/StatusSection.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `StatusSection`?"


## Interview Questions

- Walk me through StatusSectionProps line by line.
...
```bash
rg "StatusSection" apps packages --glob '*.{ts,tsx}'
```
