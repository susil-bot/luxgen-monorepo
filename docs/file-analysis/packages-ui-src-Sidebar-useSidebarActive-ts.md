...
`packages/ui/src/Sidebar/useSidebarActive.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useSidebarActive`?"

### `checkItem`

- **Approx. line:** 34
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `checkItem`?"
...
- Walk me through ActiveState line by line.
...
```bash
rg "useSidebarActive" apps packages --glob '*.{ts,tsx}'
```
