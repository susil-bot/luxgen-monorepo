...
`packages/ui/src/Sidebar/useSidebarSearch.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `useSidebarSearch`?"

### `filterItem`

- **Approx. line:** 24
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `filterItem`?"
...
- Walk me through useSidebarSearch line by line.
...
```bash
rg "useSidebarSearch" apps packages --glob '*.{ts,tsx}'
```
