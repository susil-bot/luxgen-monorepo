...
`packages/ui/src/Snackbar/Snackbar.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `getTypeStyles`?"
...
- **Approx. line:** 144
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `getIcon`?"


## Interview Questions

- Walk me through SnackbarType line by line.
...
```bash
rg "Snackbar" apps packages --glob '*.{ts,tsx}'
```
