...
`packages/ui/src/Snackbar/SnackbarProvider.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `useSnackbar`?"
...
- Walk me through useSnackbar line by line.
...
```bash
rg "SnackbarProvider" apps packages --glob '*.{ts,tsx}'
```
