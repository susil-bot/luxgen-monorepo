...
`packages/ui/src/SocialLoginButtons/SocialLoginButtons.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `SocialLoginButtons`?"


## Interview Questions

- Walk me through SocialLoginButtonsProps line by line.
...
```bash
rg "SocialLoginButtons" apps packages --glob '*.{ts,tsx}'
```
