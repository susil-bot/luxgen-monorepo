...
`packages/ui/src/SplitPageLayout/SplitPageHeader.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `SplitPageHeader`?"


## Interview Questions

- Walk me through SplitPageHeaderProps line by line.
...
```bash
rg "SplitPageHeader" apps packages --glob '*.{ts,tsx}'
```
