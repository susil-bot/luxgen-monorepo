...
`packages/ui/src/SplitPageLayout/SplitPageLayout.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `SplitPageLayout`?"


## Interview Questions

- Walk me through SplitPageLayoutProps line by line.
...
```bash
rg "SplitPageLayout" apps packages --glob '*.{ts,tsx}'
```
