...
`packages/ui/src/TenantProvider.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `useTenant`?"

### `detectTenant`

- **Approx. line:** 88
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `detectTenant`?"
...
- Walk me through useTenant line by line.
...
```bash
rg "TenantProvider" apps packages --glob '*.{ts,tsx}'
```
