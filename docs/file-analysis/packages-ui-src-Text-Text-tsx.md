...
- **Approx. line:** 51
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `getVariantSize`?"

### `getVariantWeight`

- **Approx. line:** 64
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `getVariantWeight`?"


## Interview Questions

- Walk me through TextProps line by line.
...
```bash
rg "Text" apps packages --glob '*.{ts,tsx}'
```
