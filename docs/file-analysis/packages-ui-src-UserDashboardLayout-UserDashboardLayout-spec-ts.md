# UserDashboardLayout.spec.ts

## File Path

`packages/ui/src/UserDashboardLayout/UserDashboardLayout.spec.ts`
...
- Walk me through UserDashboardLayout.spec.ts line by line.
...
```bash
rg "UserDashboardLayout.spec" apps packages --glob '*.{ts,tsx}'
```
