...
`packages/ui/src/context/NavigationContext.tsx`
...
- **Interview:** "What are inputs, outputs, and side effects of `NavigationProvider`?"

### `useNavigation`

- **Approx. line:** 19
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `useNavigation`?"
...
- Walk me through NavigationContextValue line by line.
...
```bash
rg "NavigationContext" apps packages --glob '*.{ts,tsx}'
```
