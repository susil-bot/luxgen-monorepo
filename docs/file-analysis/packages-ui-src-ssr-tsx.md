...
- **Interview:** "What are inputs, outputs, and side effects of `renderComponentToString`?"

### `getServerStyles`

- **Approx. line:** 27
- **Role:** Implementation detail — open source file for full signature.
- **Interview:** "What are inputs, outputs, and side effects of `getServerStyles`?"


## Interview Questions

- Walk me through SSRProps line by line.
...
```bash
rg "ssr" apps packages --glob '*.{ts,tsx}'
```
