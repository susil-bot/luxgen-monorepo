...
`packages/ui/src/utils/extractErrorMessage.ts`
...
- **Interview:** "What are inputs, outputs, and side effects of `extractErrorMessage`?"


## Interview Questions

- Walk me through extractErrorMessage line by line.
...
```bash
rg "extractErrorMessage" apps packages --glob '*.{ts,tsx}'
```
