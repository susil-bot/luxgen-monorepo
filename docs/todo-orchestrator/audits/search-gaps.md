...
**Bottom line:** Nav search + `/search` page return **courses + users** via GraphQL presenter. No Cmd+K overlay, command palette, AI search, saved/recent/pinned, analytics, or multi-type results (workflows/orders/products/settings).
...
| 2 | Global Search — header bar | partial | `NavBar` + `SearchBar`; `useAppLayoutHeader().onSearch` | Placeholder not “Search anything…⌘K”; no ⌘K badge; submit navigates (full page) instead of overlay |
| 2a | Global Search — overlay (filters + results) | missing | — | No modal/overlay; no type filters sidebar |
...
| UI | `packages/ui/src/SearchBar/SearchBar.tsx` | Nav/list search input |
