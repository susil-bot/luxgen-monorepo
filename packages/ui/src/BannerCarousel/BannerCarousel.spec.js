...
jest.mock('../Arrow', () => ({
  Arrow: ({ direction, onClick, 'aria-label': ariaLabel }) => (
    <button onClick={onClick} aria-label={ariaLabel} data-testid={`arrow-${direction}`}>
      {direction} arrow
    </button>
  ),
}));
...
describe('BannerCarousel Component', () => {
  ...
  describe('CTA Button', () => {
    ...
    it('calls slide ctaOnClick when provided', async () => {
      ...
      const slidesWithOnClick = [{ ...mockSlides[0], ctaOnClick: slideOnClick }];
  ...
  describe('Styling', () => {
    ...
    it('applies background image when provided', () => {
      const slidesWithImage = [{ ...mockSlides[0], backgroundImage: '/test-image.jpg' }];
  ...
  describe('Edge Cases', () => {
    it('handles slides with minimal data', () => {
      const minimalSlides = [{ id: '1', title: 'Minimal Slide' }];
