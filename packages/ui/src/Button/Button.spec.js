...
describe('Button', () => {
  ...
  it('sets type button from default prop of inputKind', () => {
    render(<Button {...fixture} inputKind={undefined} onClickHandler={mockClick} />);
    expect(screen.getByRole('button')).toHaveAttribute('type', 'button');
  });

  it('does not overwrite the given class name of button', () => {
    render(<Button {...fixture} onClickHandler={mockClick} className="bip bazz" />);
    expect(screen.getByRole('button')).toHaveClass('button', 'button--primary', 'bip', 'bazz');
  });

  it('renders data attributes on button', () => {
    render(<Button {...fixture} dataAttrs={{ 'data-foo': 'foo', 'data-bar': 'bar' }} onClickHandler={mockClick} />);
  ...
  it('does not attach aria-describedby attributes on button if ariaDescribedby value is empty', () => {
    render(<Button {...fixture} ariaDescribedby="" onClickHandler={mockClick} />);
    expect(screen.getByRole('button')).not.toHaveAttribute('aria-describedby');
  });

  it('does attach aria-describedby attributes on button if ariaDescribedby value is valid', () => {
    render(<Button {...fixture} ariaDescribedby="1234" onClickHandler={mockClick} />);
    expect(screen.getByRole('button')).toHaveAttribute('aria-describedby', '1234');
  });

  it('does attach aria-labelledby attributes on button if ariaLabelledby value is valid', () => {
    render(<Button {...fixture} ariaLabelledby="1234" onClickHandler={mockClick} />);
    expect(screen.getByRole('button')).toHaveAttribute('aria-labelledby', '1234');
  });

  it('does not attach aria-labelledby attributes on button if ariaLabelledby value is empty', () => {
    render(<Button {...fixture} ariaLabelledby="" onClickHandler={mockClick} />);
  ...
  it('adds disabled state', () => {
    render(<Button {...fixture} isDisabled={true} onClickHandler={mockClick} />);
  ...
  it('renders an optional form property', () => {
    render(<Button {...fixture} form="newsletter-submit" onClickHandler={mockClick} />);
    expect(screen.getByRole('button')).toHaveAttribute('form', 'newsletter-submit');
  });

  describe('with prop inputKind to change HTML tag', () => {
    it('renders a <button/> tag for inputKind `button`', () => {
      render(<Button {...fixture} inputKind="button" onClickHandler={mockClick} />);
      expect(screen.getByRole('button')).toBeInTheDocument();
    });

    it('renders an <a/> tag for inputKind `link`', () => {
      render(<Button {...fixture} inputKind="link" onClickHandler={mockClick} />);
      expect(screen.getByRole('link')).toBeInTheDocument();
    });

    it('renders an <button/> tag for inputKind `submit`', () => {
      render(<Button {...fixture} inputKind="submit" onClickHandler={mockClick} />);
      expect(screen.getByRole('button')).toBeInTheDocument();
    });

    it('renders a <button/> tag for inputKind `reset`', () => {
      render(<Button {...fixture} inputKind="reset" onClickHandler={mockClick} />);
      expect(screen.getByRole('button')).toBeInTheDocument();
    });

    it('renders a <div> tag for inputKind `text`', () => {
      render(<Button {...fixture} inputKind="text" onClickHandler={mockClick} />);
  ...
  describe('with inputKind `link`-only props', () => {
    it('does not add props associated with anchor tags', () => {
      render(<Button {...fixture} inputKind="button" rel="nofollow" target="_blank" onClickHandler={mockClick} />);
    ...
    it('does add props associated with anchor tags', () => {
      render(
        <Button {...fixture} inputKind="link" href="#" rel="nofollow" target="_blank" onClickHandler={mockClick} />,
      );
  ...
  describe('with ButtonIcon', () => {
    it('renders icon button only when "isIconButton" prop is set to true', () => {
      render(<Button {...fixture} ButtonIcon={DummyIcon} isIconButton={true} onClickHandler={mockClick} />);
      expect(screen.getByRole('button')).toBeTruthy();
    });

    it('renders icon', () => {
      render(<Button {...fixture} ButtonIcon={DummyIcon} hasEnableIcon={true} onClickHandler={mockClick} />);
      expect(screen.getByRole('button')).toBeTruthy();
    });
  });
});
