import type { ProductSeo } from '../../fetcher';
import { ProductEditSection } from '../../ProductEditSection';
import { buildSeoPreviewUrl, deriveUrlHandle } from './fetcher';

export interface SearchListingSectionProps {
  seo: ProductSeo;
  productTitle: string;
  tenant: string;
...
export function SearchListingSection({ seo, productTitle, tenant, onSeoChange }: SearchListingSectionProps) {
  const handle = deriveUrlHandle(productTitle, seo.urlHandle);
  ...
  const previewUrl = buildSeoPreviewUrl(tenant, handle);
