  ...
  return (
    <div className="todo-list">
      ...
      <form
        ...
        style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 0.25rem' }}
      >
...
export const TodoStatsChart: React.FC<TodoStatsChartProps> = ({ doneCount, todoCount }) => {
  ...
  const bars = [
    { label: 'To do', value: todoCount, color: 'var(--color-fill-tertiary)', textColor: 'var(--color-text-primary)' },
    {
      ...
      textColor: 'var(--color-accent, #0A84FF)',
    },
  ];
        ...
        {tiles.map((tile) => (
          <Card key={tile.label} variant="outlined" padding="medium">
            ...
            <p
              ...
              style={{ color: tile.color ?? 'var(--color-text-primary)', marginTop: '0.25rem' }}
            >
              {tile.value}
            </p>
          </Card>
